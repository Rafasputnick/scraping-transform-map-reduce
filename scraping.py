...
class PokemonScrapper(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        pokemon_details['Name'] = normalize_str(div_titulo.css(
            ".pokedex-pokemon-pagination-title div::text").extract_first())
        pokemon_details['Number'] = normalize_str(
            div_titulo.css("span.pokemon-number::text").extract_first())

        pokemon_evolution_list = response.css(".evolution-profile > li")
        current_pokemon = False
        already_satisfied = False
        for li in pokemon_evolution_list:
            evolution_name_list = li.css("a h3::text").extract()
            evolution_name_list = list(map(normalize_str, evolution_name_list))
            evolution_name_list = [
                name for name in evolution_name_list if name != '']
            evolution_name = ', '.join(evolution_name_list)

            evolution_number_list = li.css("a h3 span::text").extract()
            evolution_number_list = list(
                map(normalize_str, evolution_number_list))
            evolution_number_list = [
                number for number in evolution_number_list if number != '']
            evolution_number = ', '.join(evolution_number_list)
        ...
        for i in range(5):
            detail = info_table[i]
            detail_name = normalize_str(detail.css(
                "span.attribute-title::text").extract_first())
            if detail_name == "Gender":
                ...
                detail_value = normalize_str(detail.css(
                    "span.attribute-value::text").extract_first())
        ...
        pokemon_path = response.css(
            "div.pokedex-pokemon-pagination a.next::attr(href)").extract_first()
        pokemon_name = pokemon_details['Name']
        if pokemon_name == LAST_POKEMON_NAME:
            self.next_pokemon_url = None
        else:
            self.next_pokemon_url = response.urljoin(pokemon_path)
    ...
    def parse_pokedex_color(self, response):
        pokemon_details = response.meta['item']
        color = response.css(
            "div[data-source=color] font::text").extract_first()
        pokemon_details['Pokedex Color'] = color
        yield (pokemon_details)
        if self.next_pokemon_url != None:
            yield scrapy.Request(self.next_pokemon_url, callback=self.parse)
